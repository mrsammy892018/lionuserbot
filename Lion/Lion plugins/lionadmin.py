...
async def promote(promt):
    ...
    lionevent = await edit_or_reply(promt, "`Promoting...`")
...
async def demote(dmod):
    ...
    if not admin and not creator:
        ...
        return
    lionevent = await edit_or_reply(dmod, "`Demoting...`")
    rank = "admeme"
...
async def ban(bon):
    ...
    try:
        ...
        return
    if reason:
        await lionevent.edit(f"`{str(user.id)}` is banned !!\nReason: {reason}")
    else:
        await lionevent.edit(f"`{str(user.id)}` is banned !!")
    if BOTLOG:
...
async def nothanos(unbon):
    ...
    if not admin and not creator:
        ...
        return
    lionevent = await edit_or_reply(unbon, "`Unbanning...`")
    ...
    try:
        ...
        await lionevent.edit("```Unbanned Successfully. Granting another chance.```")
        if BOTLOG:
...
async def kick(usr):
    ...
    if not user:
        ...
        return
    lionevent = await edit_or_reply(usr, "`Kicking...`")
    try:
