...
async def calionlst_of_files(path):
    files = []
...
async def upload(path, event, udir_event, lionflag=None):
    global uploaded
    ...
    if os.path.isdir(path):
        ...
        for file in Files:
            lionpath = os.path.join(path, file)
            await upload(lionpath, event, udir_event)
...
async def uploadir(event):
    ...
    if os.path.isdir(path):
        await edit_or_reply(udir_event, f"`Gathering file details in directory {path}`")
        uploaded = 0
        await upload(path, event, udir_event, lionflag=True)
        end = datetime.now()
        ms = (end - start).seconds
        await udir_event.edit(
            f"`Uploaded {uploaded} files successfully in {ms} seconds. `"
...
async def video_lionfile(event):
    ...
    if input_str:
        ...
        if not os.path.exists(path):
            ...
            return
        lionevent = await edit_or_reply(event, "`Converting to video note..........`")
        filename = os.path.basename(path)
        lionfile = os.path.join("./temp", filename)
        copyfile(path, lionfile)
    else:
        if not reply:
    ...
    if not lionfile.endswith((".mp4", ".tgs", ".mp3", ".mov", ".gif", ".opus")):
        os.remove(lionfile)
        await edit_delete(lionevent, "```Supported Media not found...```", 5)
        return
    if lionfile.endswith((".mp4", ".tgs", ".mov", ".gif")):
        if lionfile.endswith((".tgs")):
            hmm = await make_gif(lionevent, lionfile)
            if hmm.endswith(("@tgstogifbot")):
                os.remove(lionfile)
                return await lionevent.edit(hmm)
        ...
        if aspect_ratio != 1:
            ...
            await runcmd(f'ffmpeg -i {lionfile} -vf "crop={crop_by}:{crop_by}" {PATH}')
        else:
    ...
    if os.path.exists(PATH):
        ...
        await event.client.send_file(
            ...
            progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                progress(d, t, lionevent, c_time, "Uploading...", PATH)
            ),
        )
        os.remove(PATH)
