...
@bot.on(admin_cmd(outgoing=True, pattern=r"lionuserbot$"))
@bot.on(sudo_cmd(pattern="lionuserbot$", allow_sudo=True))
async def upstream(event):
    event = await edit_or_reply(event, "`Pulling the lionuserbot repo wait a sec ....`")
    off_repo = "https://github.com/Mdnoor786/lionuserbot"
    lioncmd = f"rm -rf .git"
    try:
        await runcmd(lioncmd)
    except BaseException:
        pass
    try:
        txt = "`Oops.. Updater cannot continue due to "
...
CMD_HELP.update(
    {
        ...
        "\n\n  •  **Syntax : **`.lionuserbot`"
        "\n  •  **Function :** Shifts from official  repo to Mdnoor repo(for gali commands)"
    }
)
