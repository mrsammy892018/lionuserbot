...
async def _(event):
    if event.fwd_from:
        return
    lionevent = await edit_or_reply(event, "`pasting to del dog.....`")
...
async def _(event):
    if event.fwd_from:
        return
    lionevent = await edit_or_reply(event, "`pasting to neko bin.....`")
    input_str = "".join(event.text.split(maxsplit=1)[1:])
    if input_str:
        message = input_str
    ...
    await lionevent.edit(reply_text)
...
async def _(event):
    ...
    if r["isUrl"]:
        ...
        await lionevent.edit("code is pasted to {}".format(url))
...
async def get_dogbin_content(dog_url):
    ...
    lionevent = await edit_or_reply(dog_url, "`Getting dogbin content...`")
    ...
    if message.startswith(format_view):
        ...
        await lionevent.edit("`Is that even a dogbin url?`")
        return
    ...
    try:
        ...
        await lionevent.edit("Request timed out." + str(TimeoutErr))
        return
...
async def _(event):
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await lionevent.edit("```Please unblock me (@chotamreaderbot) u Nigga```")
            return
