"""
credits to @LiMiTLeSS786 and @simpleboy786
"""
#    Copyright (C) 2020  mdnoor786
...
async def lionbot(lionmemes):
    replied = await memes.get_reply_message()
    ...
    if not replied:
        ...
        return
    if replied.media:
        lionmemmes = await edit_or_reply(lionmemes, "passing to telegraph...")
    else:
        await edit_or_reply(lionmemes, "reply to a supported media file")
        return
    try:
        ...
        pass
    download_location = await lionmemes.client.download_media(replied, "./temp/")
    if download_location.endswith((".webp")):
        download_location = convert_toimage(download_location)
    size = os.stat(download_location).st_size
    if download_location.endswith((".jpg", ".jpeg", ".png", ".bmp", ".ico")):
        if size > 5242880:
            await lionmemmes.edit(
                "the replied file size is not supported it must me below 5 mb"
            )
            os.remove(download_location)
            return
        await lionmemmes.edit("generating image..")
    else:
        await lionmemmes.edit("the replied file is not supported")
        os.remove(download_location)
        return
    try:
        ...
        await lionmemmes.edit("ERROR: " + str(exc))
        os.remove(download_location)
        return
    lion = f"https://telegra.ph{response[0]}"
    lion = await threats(lion)
    await lionmemmes.delete()
...
async def lionbot(lionmemes):
    ...
    try:
        ...
        pass
    download_loion = await lionmemes.client.download_media(replied, "./temp/")
    if download_location.endswith((".webp")):
        download_location = convert_toimage(download_location)
    size = os.stat(download_location).st_size
    if download_location.endswith((".jpg", ".jpeg", ".png", ".bmp", ".ico")):
...
async def lionbot(lionmemes):
    input_str = lionmemes.pattern_match.group(1)
