...
async def spammer(e):
    ...
    try:
        ...
        pass
    lion = ("".join(e.text.split(maxsplit=1)[1:])).split(" ", 1)
    counter = int(lion[0])
    if counter > 50:
    ...
    if len(lion) == 2:
        ...
        if os.path.exists(downloaded_file_name):
            ...
            for _ in range(counter):
                if akay:
                    akay = await e.client.send_file(e.chat_id, akay)
                else:
                    akay = await e.client.send_file(e.chat_id, downloaded_file_name)
                try:
                    await e.client(
                        functions.messages.SaveGifRequest(
                            id=types.InputDocument(
                                id=akay.media.document.id,
                                access_hash=akay.media.document.access_hash,
                                file_reference=akay.media.document.file_reference,
                            ),
                            unsave=True,
                        )
                    )
