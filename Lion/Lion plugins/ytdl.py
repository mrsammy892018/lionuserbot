# Thanks to  for @LiMiTLeSS786 this plugin
# Instadl by @simpleboy786
# youtube plugin for lionuserbot
import asyncio
import os
import re
import time
...
async def download_video(v_url):
    ...
    try:
        ...
        return
    c_time = time.time()
    lionthumb = Path(f"{ytdl_data['id']}.jpg")
    if not os.path.exists(lionthumb):
        lionthumb = Path(f"{ytdl_data['id']}.webp")
    if not os.path.exists(lionthumb):
        lionthumb = None
    if song:
        await v_url.edit(
