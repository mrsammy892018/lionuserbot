#    Copyright (C) 2020  s.n(π.$)
# button post makker for lionuserbot thanks to uniborg for the base
# by @simpleboy786 (@LiMiTLeSS786)
import os
import re
...
async def _(event):
    if event.fwd_from:
        return
    reply_to_id = None
    lioninput = "".join(event.text.split(maxsplit=1)[1:])
    if event.reply_to_msg_id:
        reply_to_id = event.reply_to_msg_id
    await event.get_reply_message()
    # soon will try to add media support
    if not lioninput:
        lioninput = (await event.get_reply_message()).text
    if not lioninput:
        await edit_or_reply(event, "`Give me something to write in bot inline`")
        return
    lioninput = "Inline buttons " + lioninput
    tgbotusername = Config.TG_BOT_USER_NAME_BF_HER
    results = await bot.inline_query(tgbotusername, lioninput)
...
CMD_HELP.update(
    ...
    \n  •  **Example :** `.cbutton test [google]<buttonurl:https://www.google.com> [lionuserbot]<buttonurl:https://t.me/lionuserbot786:same> [support]<buttonurl:https://t.me/lionuserbot_support>`\
    \n\n  •  **Syntax : **`.ibutton`\
    \n  •  **Function :** __Buttons must be in the format as [Name on button]<buttonurl:link you want to open>__\
    \n  •  **Example :** `.ibutton test [google]<buttonurl:https://www.google.com> [lionuserbot]<buttonurl:https://t.me/lionuserbot786:same> [support]<buttonurl:https://t.me/lionuserbot_support>`\
    "
    }
)
