...
from . import ALIVE_NAME, BOTLOG, BOTLOG_CHATID, lionmemes
...
async def who(event):
    ...
    caption = await lionmemes.slap(replied_user, event, DEFAULTUSER)
...
async def decide(event):
    ...
    await event.delete()
    mdnoor = await event.client.send_message(
...
async def source(e):
    await edit_or_reply(
        e,
        "Click [here](https://github.com/Mdnoor786/lionuserbot) to open this lit af repo.",
    )
...
CMD_HELP.update(
    {
        ...
        \n\n  •  **Syntax :** `.repo`\
        \n  •  **Function : **Shows to source code link of lionuserbot.\
        \n\n  •  **Syntax :** `.lfy <query>`\
