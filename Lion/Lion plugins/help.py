...
async def cmd_list(event):
    ...
    if input_str == "text":
        string = (
            "Total {count} commands found in {plugincount} plugins of lionuserbot\n\n"
        )
        lioncount = 0
        plugincount = 0
        for i in sorted(CMD_LIST):
            ...
            string += "\n"
        if len(string) > 4095:
            data = string.format(count=lioncount, plugincount=plugincount)
            key = (
                requests.post(
            ...
            reply_text = f"**All commands of the lionuserbot can be seen [here]({url})**"
            await event.edit(reply_text)
            return
        await event.edit(string.format(count=lioncount, plugincount=plugincount))
        return
    if input_str:
        if input_str in CMD_LIST:
            ...
            for i in CMD_LIST[input_str]:
                ...
                lioncount += 1
            await event.edit(
                string.format(count=lioncount, input_str=input_str), parse_mode="HTML"
            )
        else:
        ...
        if HELPTYPE is True:
            ...
            for i in sorted(CMD_LIST):
                ...
                string += " "
                count += 1
            await event.edit(string.format(count=lioncount), parse_mode="HTML")
...
async def info(event):
    input_str = event.pattern_match.group(1)
    if input_str == "text":
        string = "Total {count} commands found in {plugincount} sudo plugins of lionuserbot\n\n"
        lioncount = 0
        plugincount = 0
        ...
        if len(string) > 4095:
            ...
            url = f"https://nekobin.com/{key}"
            reply_text = f"All commands of the lionuserbot are [here]({url})"
            await event.reply(reply_text, link_preview=False)
            return
        await event.reply(
            string.format(count=lioncount, plugincount=plugincount), link_preview=False
        )
        return
    if input_str:
        ...
        for i in sorted(SUDO_LIST):
            ...
            string += " "
            lioncount += 1
        await event.reply(string.format(count=lioncount), parse_mode="HTML")
