# modified and developed by @simpleboy786 and @akborana1
import asyncio
import base64
import io
import math
...
def pack_name(userid, pack, is_anim):
    if is_anim:
        return f"lionuserbot_{userid}_{pack}_anim"
    return f"llionuserbot_{userid}_{pack}"


def char_is_emoji(character):
    return character in lionemoji.UNICODE_EMOJI
...
async def get_pack_info(event):
    ...
    if not isinstance(stickerset_attr, DocumentAttributeSticker):
        await lionevent.edit("`This is not a sticker. Reply to a sticker.`")
        return
    get_stickerset = await event.client(
        GetStickerSetRequest(
            InputStickerSetID(
...
async def cb_sticker(event):
    ...
    if not results:
        await edit_delete(lionevent, "`No results found :(.`", 5)
        return
